...
export class WindowComponent {
    ...
    ngOnChanges(changes) {
        OFFSET_STYLES.forEach((style) => {
            if (isChanged(style, changes)) {
                this.updateOffset(style, this.options[style]);
            }
        });
        ...
        this.windowSubscription.add(this.service.change.subscribe((ev) => {
            OFFSET_STYLES.forEach((style) => {
                if (isPresent(ev[style])) {
                    this.updateOffset(style, ev[style]);
    ...
    setInitialOffset() {
        ...
        if (!isPresent(this.left) && !isPresent(this.top) && this.state !== 'maximized') {
            this.service.center();
        }
    }
    updateAllOffset() {
        OFFSET_STYLES.forEach((style) => {
            if (isPresent(this[style])) {
                this.updateOffset(style, this[style]);
            }
        });
    }
    updateOffset(style, value) {
...
WindowComponent.propDecorators = {
    ...
    'wrapperFocusedClass': [{ type: HostBinding, args: ['class.k-state-focused',] },],
